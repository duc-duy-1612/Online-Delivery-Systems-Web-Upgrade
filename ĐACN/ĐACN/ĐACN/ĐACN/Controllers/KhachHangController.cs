...
namespace ĐACN.Controllers
{
    public class KhachHangController : BaseController
    {
        ...
        [HttpPost]
        public ActionResult DatHang(string maNH, string diaChi, string phuongXa, string quanHuyen, string tinhTP, string sdt, string phuongThucTT)
        {
            if (!KiemTraDangNhap()) { TempData["Msg"] = "Vui lòng đăng nhập!"; return RedirectToAction("TrangChu", "Home"); }
            string maKH = Session["MaKH"] as string;
            ...
            var cart = db.LichSuGioHangs.Where(x => x.MaKH == maKH && x.MaNH == maNH).ToList();
            if (!cart.Any()) { TempData["Msg"] = "Giỏ hàng trống!"; return RedirectToAction("XemGioHang"); }
