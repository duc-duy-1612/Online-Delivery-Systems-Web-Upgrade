    ...
    public class HomeController : BaseController
    {
        ...
        private List<NhaHangViewModel> LoadNhaHangData(double? userLat = null, double? userLng = null)
        {
            ...
            var nhaHangList = db.NhaHangs
                ...
                .ToList();
            var danhGiaList = db.DanhGiaNhaHangs.Select(dg => new { dg.MaNH, SoSao = dg.SoSao ?? 0 }).ToList();
            var luotMuaDict = db.DonHangs.GroupBy(d => d.MaNH).Select(g => new { MaNH = g.Key, LuotMua = g.Select(d => d.MaDon).Distinct().Count() }).ToList();
            var maxLuotMua = luotMuaDict.Any() ? luotMuaDict.Max(l => l.LuotMua) : 1;

            return nhaHangList.Select(x =>
            {
                if ((x.Latitude == null || x.Latitude == 0) && !string.IsNullOrEmpty(x.DiaChi))
                {
                    var coords = GeoCodeORS(x.DiaChi);
                    if (coords.lat.HasValue) { x.Latitude = coords.lat; x.Longitude = coords.lng; db.Entry(x).State = EntityState.Modified; db.SaveChanges(); }
                }
                double rating = danhGiaList.Where(dg => dg.MaNH == x.MaNH).Select(dg => (double?)dg.SoSao).DefaultIfEmpty(0).Average() ?? 0;
